...
//                }
    ...
    private static void bust(boolean instaBust) {
        ...
        for (String bustP : activePlayerMap.keySet()) {
            try {
                UUID uuidBust = UUID.fromString(bustP);
                playSoundEffect(BustaMine.plugin.getServer().getPlayer(uuidBust), "Bust");
            } catch (Exception ignored) {
            }
        }
    ...
    // 소리 재생
    public static void playSoundEffect(Player player, String key) {
        try {
    ...
    public static void runCommandRoundStart() {
        if (BustaMine.ccConfig.get().getString("Command.WhenRoundStart", "").isEmpty()) return;
    ...
    public static void runCommandBet(Player p, int amount) {
        if (BustaMine.ccConfig.get().getString("Command.WhenPlayerBet", "").isEmpty()) return;
    ...
    public static void runCommandCashOut(Player p, double amount, int multiplier, double prize) {
        if (BustaMine.ccConfig.get().getString("Command.WhenPlayerCashOut", "").isEmpty()) return;
    ...
    public static void runCommandRoundEnd(int multiplier) {
        if (BustaMine.ccConfig.get().getString("Command.WhenRoundEnd", "").isEmpty()) return;
    ...
    public static void gameGUISetup() {
        closeAllGameUI();
        createGameGUI(bustaType.money);
        createGameGUI(bustaType.exp);
    }

    private static void createGameGUI(bustaType type) {
        // UI 요소 생성
    ...
    public static void openGameGUI(Player p, bustaType type) {
        if (type == bustaType.money) {
    ...
    public static void top(Player p, String type) {
        ...
        for (String s : sortedMap.get(type).keySet()) {
            try {
                ...
                p.sendMessage(BustaMine.prefix + "Failed to load player data. " + s);
            }
        }
